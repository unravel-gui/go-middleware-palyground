...
type RaftLog struct {
	Entries            []LogEntry
	CommitIndex        int64 // 提交进度
	Applied            int64 // 应用于状态机进度
...
func NewRaftLog(persist *Persister, maxNextEntriesSize int64) *RaftLog {
	...
	if raftLogs == nil || len(raftLogs) == 0 {
		rl.Entries = []LogEntry{{Term: 0, Index: 0, Data: nil}}
		rl.CommitIndex = 0
		rl.Applied = 0
	} else {
		// 存在runtime data
...
func (rl *RaftLog) FindConflict(prevLogIndex int64, prevLogTerm int64) int64 {
	last := rl.lastIndex()
...
func (rl *RaftLog) GetEntries(index int64) []LogEntry {
	lastIndex := rl.lastIndex()
	if index > lastIndex {
		return nil
	}
	return rl.RaftLogSlice(index, lastIndex+1, rl.MaxNextEntriesSize)
}
