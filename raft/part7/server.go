...
type Server struct {
	...
	peerClients map[int64]*rpc.Client // 其他节点的客户端，用于调度其他节点的rpc服务
...
func NewServer(serverId int64, endpoint string, peerIds map[int64]string, storage Storage, ready <-chan interface{}, commitChan chan<- CommitEntry) *Server {
	s := new(Server)
	s.serverId = serverId
	s.endpoint = endpoint
	s.peerIds = peerIds
...
func (s *Server) Serve() {
	s.mu.Lock()
	// 创建共识模块
	s.cm = NewConsensusModule(s.serverId, s.endpoint, s.peerIds, s, s.storage, s.ready, s.commitChan)
	// 创建rpc服务器(开启RPC服务)
