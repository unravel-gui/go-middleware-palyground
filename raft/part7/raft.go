...
type ConsensusModule struct {
	...
	id       int64 // 自身id
	endpoint string
	peerIds  map[int64]string // 其他节点的id
	...
	lastApplied        int64     // 已经用于状态机的日志索引
...
func NewConsensusModule(id int64, endpoint string, peerIds map[int64]string, server *Server, storage Storage,
...
type RequestVoteArgs struct {
	Term         int64 // 请求方任期
	CandidateId  int64 // 候选人Id(请求方)
	LastLogIndex int64 // 请求方最新的日志索引
	LastLogTerm  int64 // 请求方最新的日志任期
}
...
func (cm *ConsensusModule) lastLogIndexAndTerm() (int64, int64) {
	if len(cm.log) > 0 {
		...
		return int64(lastIndex), int64(cm.log[lastIndex].Term)
	} else {
		return -1, -1
	}
}
